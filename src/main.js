...
function getElement(selector) {
  ...
  if (!Element) console.error(`element "${selector}" does not exist!`);
...
function getElements(selector) {
  ...
  if (Elements.length === 0) console.error(`elements "${selector}" do not exist!`);
...
function handleMenuLinkEvent() {
  getElement('.menu').addEventListener('click', (event) => {
    if (event.target.closest('a')) getElement('#menu-btn').checked = false;
  });
}
